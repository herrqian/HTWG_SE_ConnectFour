...
class FileIO extends FileIOInterface {
  override def load: (GridInterface, Array[Boolean]) = {
    ...
    val sourceFile: scala.io.BufferedSource = Source.fromFile("grid.json")
    val sourceLines: scala.collection.Iterator[String] = sourceFile.getLines()
    val sourceStr: String = sourceLines.mkString("")
    val json: JsValue = Json.parse(sourceStr)
    ...
    size match {
      ...
      case 272 =>
        grid = injector.instance[GridInterface](Names.named("Grid Huge"))
      case _ => println("jjj")
    }
