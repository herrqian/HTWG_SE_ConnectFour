...
case class Matrix[T](rows: Vector[Vector[T]]) {
  def this(row: Int, col: Int, filling: T) = this(Vector.tabulate(row, col) {
    (row, col) => filling
  })
  ...
  def fill(filling: T): Matrix[T] = copy(Vector.tabulate(row, col) {
    (row, col) => filling
  })
  ...
  override def toString: String = {
    var stringVal = ""
    rows.foreach { x =>
      x foreach { x => stringVal = stringVal + x + " " }
